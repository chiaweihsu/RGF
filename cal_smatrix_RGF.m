...
%
%   Basic usage:
%   smatrix = CAL_SMATRIX_RGF(syst, out, in);
%
%   For additional options and information, use:
%   [smatrix, channels] = CAL_SMATRIX_RGF(syst, out, in);
%
%   Returns portions of the scattering matrix with output and input channels
%   specified by out and in, for the system specified by syst. The system is
%   Maxwell's equations for 2D TM modes:
...
%       syst.BC (character vector or string, case insensitive):
%           Boundary condition in the transverse direction y; can be 'periodic'
%           or 'Dirichlet'. Periodic BC means E_z(n,m+ny)=E_z(n,m). Dirichlet BC
%           means E_z(n,m=0)=E_z(n,m=ny+1)=0.
...
% Assemble ii = row index, jj = column index, vv = value for the non-zero entries of A0
ind_D  = 1:ny;  % diagonal (d=0)
ind_dy = 1:(ny-1);  % vertical links along y, at diagonals with d=1,-1
if strcmpi(syst.BC, 'Dirichlet')
    ii = [ind_D, ind_dy+1,  ind_dy ];
    jj = [ind_D,  ind_dy , ind_dy+1];
    vv = [-4*ones(1,ny), ones(1, 2*ny-2)];
else % periodic
    ind_dy_periodic = ny; % vertical links connecting top back to bottom, at diagonals with d=ny-1,-(ny-1)
    ii = [ind_D, ind_dy+1,  ind_dy ,   ind_dy_periodic, ind_dy_periodic-ny+1];
    jj = [ind_D,  ind_dy , ind_dy+1, ind_dy_periodic-ny+1, ind_dy_periodic  ];
    vv = [-4*ones(1,ny), ones(1, 2*ny)];
end

% Finite-difference wave operator for one slice, without the (kdx)^2*epsilon term
% Note that sparse(ii,jj,vv) adds elements of vv for duplicate subscripts in ii and jj. Therefore, the periodic BC works even when ny<3 and reduces to the 1D operator when ny=1 (where the three terms of (d/dy)^2 cancel each other).
A0 = sparse(ii, jj, vv, ny, ny);
...
elseif N_in_R==0
    ...
    G_LL = (eye(ny) - G0_L * G_LL) \ G0_L;
    G_RL = - G_RL * G_LL;
    smatrix = [C_L * (G_LL * B_L); C_R * (G_RL * B_L)];
...
elseif N_in_L==0
    ...
    G_RR = (eye(ny) - G0_R * G_RR) \ G0_R;
    G_LR = - G_LR * G_RR;
    smatrix = [C_L * (G_LR * B_R); C_R * (G_RR * B_R)];
else
    ...
    smatrix = [[C_L * (G_LL * B_L); C_R * (G_RL * B_L)], [C_L * (G_LR * B_R); C_R * (G_RR * B_R)]];
end
if issparse(smatrix); smatrix = full(smatrix); end

%% Wrapping up

% Include the 2i prefactor
smatrix = (2i)*smatrix;
